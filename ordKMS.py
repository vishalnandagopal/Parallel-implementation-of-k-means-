...
def eucl_distance(
    point_one, point_two
):  # define function to measure Euclidian distance between point_one and point_two
    if len(point_one) != len(
        point_two
    ):  # check for an error (len(1st point)is not equal len(2nd point))
        raise Exception("Error: non comparable points")  # check for an error

    sum_diff = 0.0  # set variable sum_diff
    for i in range(
        len(point_one)
    ):  # iterating take difference between each elem from 1st and 2nd point
        diff = pow(
            (float(point_one[i]) - float(point_two[i])), 2
        )  # and square this difference.
        sum_diff += diff  # As the result summing all these differencess and equating to sum_diff
    final = math.sqrt(
        sum_diff
    )  # sqrt from sum_diff will be Euqlidian dist betw(1st and 2nd points)
    return final
...
while flag == True:
    ...
    for i in range(len(dist)):  # iterable take each raw in dist matrix and
        cluster.append(
            np.argmin(dist[i]) + 1
        )  # find column index of min value (this index is number of centorud)
